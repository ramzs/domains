$(function() {
	...
	//E-mail Ajax Send
	$("form.c-form2").submit(function() { //Change
		var th = $(this);
		$.ajax({
			type: "POST",
			url: "/mail.php", //Change
			...
			setTimeout(function() {
				$(th).closest('.callback-form').find('.success').removeClass('active').fadeOut();
				window.location.replace("http://xstels-2/thank.html");
				th.trigger("reset");
			}, 2000);
		});
		return false;
	...
	//E-mail Ajax Send
	$("form.c-form3").submit(function() { //Change
		var th = $(this);
		$.ajax({
			type: "POST",
			url: "/mail.php", //Change
	...
	$('.btn-main--book').on('click', function() {
		var model       = $(this).data('model');
			program     = $(this).data('program');
			formModel   = $('span.model');
			formProgram = $('span.program');

		formModel.text(model);
