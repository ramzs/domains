var calendar = {
	...
	init: function(ajax) {
      ...
      $.ajax({
  				url: 'libs/eventcalendare/data/events.json',
  				type: 'GET',
  			})
  			.done(function(data) {
    ...
    var mon = 'Пн';
		...
		function setEvent() {
			$('.day-event').each(function(i) {
				...
				var dayActive = $('.day-event.open').attr('date-day');
				var monthActive = $('.day-event.open').attr('date-month');
				if ($('.day-event').hasClass('open')) {
					$('tbody.event-calendar tr td[date-month="' + monthActive + '"][date-day="' + dayActive + '"]').addClass('active');
				} else {
					$('tbody.event-calendar tr td').removeClass('active');
				}
			});
		};
		...
		function displayEvent() {
			$('tbody.event-calendar td').on('click', function(e) {
				...
				if ($(this).hasClass('event')) {
					$('tbody.event-calendar td').removeClass('active');
					$('.day-event').slideUp('fast').removeClass('open');
					$(this).addClass('active');
					$('.day-event[date-month="' + monthEvent + '"][date-day="' + dayEvent + '"]').slideDown('fast').addClass('open');
				} else {
					$('tbody.event-calendar td').not('.active').removeClass('active');
				};
			});
		};
